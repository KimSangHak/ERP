...
<div id="container">
	...
	<div class="popForm">
        ...
        <div class="popTitArea">
        	<h1>${item.orderFullNo } ${item.device } 프로그램 진행 상태변경
			</h1>
        </div>
        <!-- //팝업 타이틀 -->
            ...
            <div class="popList">
                ...
                <div class="popBoard">
                   	<table>
                        ...
                        <tbody>
                        	<c:if test="${item.programStatus eq 'R'}">
                        	<tr>
                        		<th>프로그램담당</th>
                        		<td>
                        	...
                        	<tr>
                        		...
                        		<td>
									<select  id="status" name="status">
										<option value="I" <c:if test="${item.programStatus eq 'I'}">selected</c:if>>작업시작</option>
										<option value="F" <c:if test="${item.programStatus eq 'F'}">selected</c:if>>작업완료</option>
										<option value="H" <c:if test="${item.programStatus eq 'H'}">selected</c:if>>홀딩</option>
										<option value="S" <c:if test="${item.programStatus eq 'S'}">selected</c:if>>중지</option>
									</select>
                        		</td>
                        	</tr>
                        	<tr>
